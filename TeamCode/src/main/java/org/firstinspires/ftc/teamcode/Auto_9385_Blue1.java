...
@Autonomous (name = "Auto Blue uno", group = "9385")
@Disabled
/**
 * Created by Emmanuel on 12/5/2017.
 */

public class Auto_9385_Blue1 extends LinearOpMode {
    ...
    public void runOpMode(){
        ...
        while (opModeIsActive()) {
            ...
            brobot.Grab1.setPosition(MAX_POSITION);
            brobot.Grab2.setPosition(MIN_POSITION);

            brobot.Raise.setPower(RAISER_POWER);
            sleep(5000);
            brobot.Raise.setPower(0);
            pause();
            ...
            //knock jewel off
            if (brobot.Color.red() >= 3){
                encoderDrive(TURN_SPEED, 3, -3, 0.5);
                pause();
                //bring up the jewel mech
                brobot.JewlKnocker.setPosition(HOME_POSITION);
                pause();
                encoderDrive(DRIVE_SPEED, -1, -1, 0.2);
                pause();
                encoderDrive(TURN_SPEED, -3, 3, 0.5);
                pause();
            } else  if(brobot.Color.blue() >= 3){
                encoderDrive(TURN_SPEED, -3, 3, 0.5);
                pause();
                //bring up the jewel mech
                brobot.JewlKnocker.setPosition(HOME_POSITION);
            ...
            encoderDrive(DRIVE_SPEED, -10, -10, 1.8);
            pause();
            encoderDrive(TURN_SPEED, 4, -4, 0.6);
            pause();
            ...
            encoderDrive(DRIVE_SPEED, 3, 3, 0.2);
            pause();
            brobot.Grab1.setPosition(MIN_POSITION);
            brobot.Grab2.setPosition(MAX_POSITION);
            pause();
            encoderDrive(DRIVE_SPEED, -3, -3, 0.2);
            pause();
